...
-- views
select * from dbo.vw_PO_Header_WaitingActive
select * from dbo.vw_PO_Line_Active
...
-- lookups
select [Amount], [Level] from dbo.PO_DelegationOfAuthority_Direct_Material order by Amount
select [Amount], [Level] from dbo.PO_DelegationOfAuthority_Indirect_Expense order by Amount

-- approver directory
truncate table dbo.PO_ApproverDirectory
select * from dbo.PO_ApproverDirectory
...
truncate table PO_ApprovalStage
select * from PO_ApprovalStage

delete from dbo.PO_ApprovalChain
select * from PO_ApprovalChain
...
select * from PO_ApproverDirectory

-- 713701 good now. --D only > 50 k and <= 100000 got BUYER should be LPM
-- 713986 good. --D only < 50k got no stages ok
-- 713987 good. --D only < 50k got no stages ok
-- 713993 good. --D only < 50k got no stages ok
-- 713997 good. --I only < 2k got first four sequences only ok
-- 714002 good. --D < 50 k got no stage ok and I < 2k first four sequences only ok
-- 714003 good. --I only < 2k got first four sequences only ok
-- 714004 good. --I < 2k first four sequences only ok D > 50 k and <= 100000 got BUYER should be LPM
-- 714009 good now. -- ****bad. --D only > 100000 and <= 249999.99 got FINANCIAL CONTROLLER (SITE) shoud be LOCAL PURCHASING MANAGER and FINANCIAL CONTROLLER (SITE) only
-- 714012 good. --D only > 50 k and <= 100000 got BUYER should be LPM
-- 714015 good now. --****bad. --D only > 100000 and <= 249999.99 got FINANCIAL CONTROLLER (SITE) shoud be LOCAL PURCHASING MANAGER and FINANCIAL CONTROLLER (SITE) only
-- 714016 good. --D < 50000 got no stage ok and I < 2k got first four sequences ok
-- 714019 good. --I only < 2k got first four sequences ok
-- 714020 good now. --****bad. --D only > 100000 and <= 249999.99 got FINANCIAL CONTROLLER (SITE) shoud be LOCAL PURCHASING MANAGER and FINANCIAL CONTROLLER (SITE) only
declare @po char(6)= '714020'
-- PO Header
select PoNumber, DirectAmount, IndirectAmount, BuyerCode, HouseCode from dbo.PO_Header where PoNumber = @po
-- PO Outbox
select * from dbo.PO_ApprovalOutbox where PoNumber = @po
-- PO Chain
select * from dbo.PO_ApprovalChain where PoNumber = @po
-- PO Approval Stage
select * from dbo.PO_ApprovalStage where PoNumber = @po